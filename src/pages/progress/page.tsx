  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        status === "completed" && "bg-primary/8 border-primary/20",
        status === "current" && "bg-accent border-primary/30 shadow-sm",
    ...
    >
      ...
      {status === "current" && (
        <span className="text-xs font-semibold text-primary bg-primary/12 px-2.5 py-1 rounded-full shrink-0">
          Today
        </span>
      )}
...
function ProgressInner() {
  ...
  for (const task of allTasks) {
    const dayTasks = tasksByDay.get(task.dayNumber) ?? [];
    tasksByDay.set(task.dayNumber, [...dayTasks, task]);
  }
  for (const [dayNum, tasks] of tasksByDay.entries()) {
    if (tasks.filter((t) => t.isCompleted).length >= 9) completedDays.add(dayNum);
  }
    ...
    <div className="px-5 pt-8 pb-4 space-y-6">
      ...
      >
        <div className="bg-card border border-border rounded-2xl p-3 text-center">
          <FlameIcon size={20} className="text-primary mx-auto mb-1" />
          <p className="text-xl font-bold text-foreground">{streak}</p>
          <p className="text-[10px] text-muted-foreground font-medium">Days Done</p>
        </div>
        <div className="bg-card border border-border rounded-2xl p-3 text-center">
          <TrophyIcon size={20} className="text-primary mx-auto mb-1" />
          <p className="text-xl font-bold text-foreground">{totalCompleted}</p>
          <p className="text-[10px] text-muted-foreground font-medium">Days Done</p>
        </div>
        <div className="bg-card border border-border rounded-2xl p-3 text-center">
          <CalendarIcon size={20} className="text-primary mx-auto mb-1" />
      ...
      >
        <div className="flex justify-between mb-3">
          ...
          <p className="text-sm font-bold text-primary">{totalCompleted}/9 days</p>
        </div>
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">
          9-Day Consciousness Journey
        </h2>
        {CHALLENGE_DAYS_FE.map((dayData, i) => {
          const dayNum = i + 1;
          const status = completedDays.has(dayNum)
            ? "completed"
            : dayNum === currentDay
              ? "current"
              : "future";
