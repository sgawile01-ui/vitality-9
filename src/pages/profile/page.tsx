...
function ProfileInner() {
  ...
  const handleManageBilling = async () => {
    ...
    try {
      const { url } = await createPortal({ returnUrl: `${window.location.origin}/profile` });
      window.location.href = url;
    } catch {
  ...
  return (
    <div className="px-5 pt-8 pb-4 space-y-6">
      ...
      <motion.div
        className="bg-card border border-border rounded-3xl p-6 flex flex-col items-center gap-3"
        initial={{ opacity: 0, scale: 0.97 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={{ delay: 0.1 }}
      >
        <div className="w-20 h-20 rounded-full bg-gradient-to-br from-primary/30 to-accent flex items-center justify-center">
      ...
      >
        ...
        <InfoRow icon={FlameIcon} label="Completed days" value={`${user?.streakCount ?? 0} days`} />
        <InfoRow
          icon={ShieldCheckIcon}
          label="Account type"
          value={user?.isPro ? "Pro Member" : "Free Plan"}
          valueClass={user?.isPro ? "text-primary font-semibold" : undefined}
        />
      </motion.div>
      {!user?.isPro && (
        <motion.div
          className="bg-gradient-to-br from-primary/10 to-accent/50 border border-primary/15 rounded-3xl p-5"
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="font-bold text-foreground">Upgrade to Pro</p>
          </div>
          <ul className="text-sm text-muted-foreground mb-4 mt-2 space-y-1">
            {["Unlimited 9-day journeys", "AI wellness coaching", "Priority support"].map((f) => (
      ...
      {user?.isPro && (
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="font-bold text-foreground">Pro Member</p>
          </div>
          <p className="text-sm text-muted-foreground mb-4">
            You have full access to all 9-day journeys and AI coaching.
          </p>
          <Button
            variant="secondary"
            className="w-full"
          ...
          >
            {portalLoading ? (
              ...
              <>
                ...
                Manage Billing
                <ExternalLinkIcon size={13} className="ml-auto opacity-50" />
              </>
            )}
          </Button>
        </motion.div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="text-sm font-semibold text-foreground">{p.theme}</span>
                  <span className="text-base">{p.icon}</span>
                </div>
                <p className="text-xs text-muted-foreground italic truncate">{p.pillar}</p>
              </div>
            </motion.div>
          ))}
        </div>
