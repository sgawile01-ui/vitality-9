...
function TaskItem({
  ...
  task: { taskText: string; isCompleted: boolean; taskIndex: number };
  index: number;
  onToggle: (taskIndex: number, isCompleted: boolean) => void;
}) {
  ...
  return (
    <motion.button
      ...
      layout
      onClick={() => onToggle(task.taskIndex, !task.isCompleted)}
      className={cn(
        "w-full flex items-center gap-3 p-4 rounded-2xl border transition-all duration-200 text-left cursor-pointer",
        task.isCompleted
          ? "bg-primary/8 border-primary/20"
          : "bg-card border-border hover:border-primary/30 hover:bg-primary/4",
      )}
      whileTap={{ scale: 0.97 }}
    >
      <div
      ...
      <AnimatePresence mode="wait">
        {task.isCompleted ? (
          ...
          >
            <CheckCircle2Icon size={22} className="text-primary shrink-0" />
          </motion.div>
        ) : (
          <motion.div
            key="unchecked"
            ...
            exit={{ scale: 0, opacity: 0 }}
          >
            <CircleIcon size={22} className="text-border shrink-0" />
          </motion.div>
        )}
      </AnimatePresence>
    </motion.button>
...
function HomeInner() {
  ...
  useEffect(() => {
    if (searchParams.get("pro") === "success") {
      toast.success("Checkout returned. Access will update after server verification.", {
        duration: 5000,
      });
  ...
  return (
    <div className="px-5 pt-8 pb-4 space-y-6">
      ...
      >
        <div>
          ...
          <h1 className="text-xl font-bold text-foreground">
            {user?.name?.split(" ")[0] ?? "Wellness Seeker"} ✨
          </h1>
        </div>
        <div className="flex items-center gap-1.5 bg-accent px-3 py-1.5 rounded-full">
          ...
          <span className="text-xs font-semibold text-accent-foreground">
            {user?.streakCount ?? 0} days completed
          </span>
        </div>
      </motion.div>
      ...
      >
        ...
        {tasks.length === 0 && (
          <p role="status">No activities are available yet. Please reload or contact support.</p>
        )}
        {tasks.map((task, index) => (
          <TaskItem key={task.taskIndex} task={task} index={index} onToggle={handleToggle} />
        ))}
      </motion.div>
      ...
      <motion.div
        ...
        className="bg-gradient-to-br from-primary/10 to-accent/60 rounded-3xl p-5 border border-primary/15"
      >
      ...
      >
        <button
          ...
          className={cn(
            "w-full flex items-center gap-3 p-4 rounded-2xl border transition-all duration-200 cursor-pointer",
            user?.isPro
              ? "bg-gradient-to-r from-primary/10 to-accent/50 border-primary/20 hover:border-primary/40"
              : "bg-card border-border hover:border-primary/20 hover:bg-primary/4",
          )}
        >
          <div
            className={cn(
              "p-2 rounded-xl shrink-0",
              user?.isPro ? "bg-primary/15 text-primary" : "bg-muted text-muted-foreground",
            )}
          >
            <BotIcon size={18} />
          </div>
          <div className="flex-1 text-left">
            <p className="text-sm font-semibold text-foreground">
              {user?.isPro ? "AI Wellness Coach" : "Chat with AI Coach"}
            </p>
      ...
      <Unauthenticated>
        <div className="flex flex-col items-center justify-center min-h-screen px-6 text-center space-y-6">
          <div className="p-4 bg-primary/10 rounded-full">
            <HeartPulseIcon size={40} className="text-primary" />
          </div>
          <h1 className="text-3xl font-bold text-foreground">Vitality 9</h1>
          <p className="text-muted-foreground text-base max-w-xs mx-auto">
            Your 9-Day Consciousness &amp; Vitality Journey. Nine pillars. One transformation.
          </p>
