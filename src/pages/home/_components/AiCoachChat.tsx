...
export default function AiCoachChat({ onClose }: { onClose: () => void }) {
  ...
  const handleSend = async (text?: string) => {
    ...
    const history = messages
      ...
      .slice(-6)
      .map((m) => ({ role: m.role, text: m.text.slice(0, 1600) }));
    try {
      ...
      if (err instanceof ConvexError) {
        ...
        if (data.code === "UNAUTHENTICATED")
          friendlyMessage = "Please sign in to use the AI Coach.";
        else if (data.code === "FORBIDDEN")
          friendlyMessage = "AI coaching requires a Pro subscription. Tap 'Get Pro' to upgrade.";
        else if (data.code === "NOT_CONFIGURED")
          friendlyMessage = "The wellness coach is not available yet. Please contact support.";
        else if (data.message) friendlyMessage = `👤 ${data.message}`;
      }
      setMessages((prev) => [
        ...prev,
        { id: crypto.randomUUID(), role: "model", text: friendlyMessage },
      ]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <motion.div
      ...
      className="flex flex-col bg-card border border-border rounded-3xl overflow-hidden shadow-lg"
      style={{ height: "520px" }}
    >
      ...
      >
        <AnimatePresence initial={false}>
          {messages.map((msg) => (
            ...
            >
              {msg.role === "model" && (
                <div className="w-7 h-7 rounded-full bg-primary/12 flex items-center justify-center shrink-0 mt-0.5">
            ...
            <div className="flex flex-wrap gap-1.5 justify-center">
              {SUGGESTED_PROMPTS.map((p) => (
                <button
                  ...
                  className="text-xs px-3 py-1.5 bg-accent hover:bg-primary/12 text-accent-foreground rounded-full border border-border transition-colors cursor-pointer"
                >
                  {p}
                </button>
              ))}
      ...
      <div className="px-3 py-3 border-t border-border shrink-0">
        ...
        <div className="flex items-end gap-2 bg-muted rounded-2xl px-3 py-2">
          <textarea
            ...
            rows={1}
            disabled={isLoading}
            className="flex-1 bg-transparent text-sm text-foreground placeholder:text-muted-foreground resize-none outline-none py-1 max-h-24"
        ...
        <p className="text-[10px] text-muted-foreground text-center mt-1.5">
          General wellness only. For urgent symptoms or immediate danger, contact local emergency
          services now.
        </p>
      </div>
    </motion.div>
