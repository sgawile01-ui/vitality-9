...
The launcher no longer sets `BETA_MODE=local`. During initialization it sets `BETA_ALLOWED_SUBJECTS` to `founder-testing,synthetic-local-check` on the anonymous local deployment only. These exact synthetic subjects are explicitly invited; every mode now enforces invitations. The legacy mode setting cannot bypass access checks. Start the testing launcher before running `npm run test:local`; that script verifies both invited non-Pro denial (`FORBIDDEN`) and uninvited denial (`BETA_RESTRICTED`). The adapter rejects hosted/production runtimes, builds, non-loopback binds and forwarded requests. Use the normal app entry point for genuine Hercules login, as described in [authentication gate](AUTHENTICATION_GATE.md).
